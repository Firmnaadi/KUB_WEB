...
if (isset($_POST['submit'])) {
 ...
 if (!empty($bln)) {
  ...
  $q = mysqli_query($conn, "SELECT * FROM `pembelian_kbth_ayam` WHERE MONTH(tanggal) = '$bln'");
 } else {
  // perintah tampil semua data
  $q = mysqli_query($conn, "SELECT * FROM `pembelian_kbth_ayam` p");
 }
} else {
 // perintah tampil semua data
 $q = mysqli_query($conn, "SELECT * FROM `pembelian_kbth_ayam`");
}
...
<body>
  ...
  <center>
   <h1>Laporan Pembelian Kebutuhan Ayam</h1>
  </center>
