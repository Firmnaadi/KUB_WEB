...
if (isset($_POST['submit'])) {
 ...
 if (!empty($bln)) {
  ...
  $q = mysqli_query($conn, "SELECT * FROM `pendapatan_telur` WHERE MONTH(tanggal) = '$bln'");
 } else {
  // perintah tampil semua data
  $q = mysqli_query($conn, "SELECT * FROM `pendapatan_telur` p");
 }
} else {
 // perintah tampil semua data
 $q = mysqli_query($conn, "SELECT * FROM `pendapatan_telur`");
}
...
<body>
  ...
  <center>
   <h1>Laporan Pendapatan Telur</h1>
  </center>
  ...
  <div class="card col-md-8 mx-auto mt-3">
   <div class="card-body">
    ...
    while($data = mysqli_fetch_array($q)) {         
        echo "<tr>";
        echo "<td>".$data['id_pendapatan_telur']."</td>";
